...
import { TREND_MODEL_VERSION, TREND_PROFILE_VERSION } from './model-versions.mjs';

const require = createRequire(import.meta.url);
const googleTrends = require('google-trends-api');

export { TREND_MODEL_VERSION, TREND_PROFILE_VERSION };
...
/**
 * Space out requests on the keyless path.
 *
 * `google-trends-api` scrapes an endpoint Google does not publish, and it answers
 * a burst with 429s. A single candidate costs two requests (30d + 90d) and up to
 * four once the WORLDWIDE retry kicks in, and `scripts/scan.mjs` walks its queue
 * with no delay between candidates — so enabling the free path without this
 * would fire a burst and collect rate-limit errors instead of verdicts.
 *
 * The gate lives here rather than in the caller so that every entry point
 * (scan.mjs, and the trend fillers) is throttled by the same clock. The paid
 * path is not throttled: SerpApi is a real API with a documented quota.
 */
let lastFreeRequestAt = 0;
async function throttleFreePath() {
  if (SERPAPI_API_KEY) return;
  const gap = Math.max(0, Number(process.env.TRENDS_FREE_DELAY_MS ?? 1200));
  const wait = lastFreeRequestAt + gap - Date.now();
