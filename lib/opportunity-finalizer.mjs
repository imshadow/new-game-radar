...
import { TREND_MODEL_VERSION, SEO_MODEL_VERSION, OPPORTUNITY_MODEL_VERSION, SITE_TYPE_MODEL_VERSION } from './model-versions.mjs';
...
function analyzeOpportunity(candidate, channel, market, wikiPrelaunch, socialBuzz, nowMs) {
...
export function typeOf(candidate) {
  if (candidate.siteType?.modelVersion === SITE_TYPE_MODEL_VERSION) return candidate.siteType.type;
...
function recentlyDiscovered(candidate, maxDays = 14, nowMs = Date.now()) {
  const firstSeen = Date.parse(candidate.firstSeen || '');
  return Number.isFinite(firstSeen) && nowMs - firstSeen <= maxDays * 86400000;
}

/**
 * Evaluate a candidate and write its final recommendation.
 *
 * `nowMs` is an explicit input rather than an implicit `Date.now()` read so the
 * whole evaluation is deterministic: the same candidate + the same clock always
 * produce the same verdict. Tests pin it to a fixture timestamp; the scanner
 * passes the scan's own `now` so every candidate in a run is judged against one
 * consistent instant.
 */
export function applyFinalRecommendation(candidate, nowMs = Date.now()) {
  ...
  const market = analyzeMarketFreshness(candidate, nowMs);
  const wikiPrelaunch = analyzeWikiPrelaunch({ ...candidate, social: socialBuzz }, nowMs);
  const opportunity = analyzeOpportunity(candidate, channel, market, wikiPrelaunch, socialBuzz, nowMs);
  ...
  const onlineFreshEnough = keywordFreshness === 'new'
    || (recentlyDiscovered(candidate, 14, nowMs) && socialBuzz.classification === 'viral');
  ...
  if (candidate.seo?.modelVersion !== SEO_MODEL_VERSION) recommendation = 'pending';
