...
import { FAST_MODEL_VERSION, SITE_TYPE_MODEL_VERSION } from './model-versions.mjs';
...
// Declared in lib/source-registry.mjs so that every consumer of a source can be
// found in one place. These used to be five separate copies of the same list,
// and adding a source to only some of them down-weighted it silently.
const SHARED_HIGH_QUALITY_KINDS = POLICY_SETS.FAST_SHARED_HIGH_QUALITY;
const ONLINE_HIGH_QUALITY_KINDS = POLICY_SETS.FAST_ONLINE_HIGH_QUALITY;
const WIKI_HIGH_QUALITY_KINDS = POLICY_SETS.FAST_WIKI_HIGH_QUALITY;
...
export function calculateFastSignals(candidate, previousFast = {}, nowMs = Date.now()) {
  ...
  const typeInfo = candidate.siteType?.modelVersion === SITE_TYPE_MODEL_VERSION ? candidate.siteType : classifySiteType(candidate);
