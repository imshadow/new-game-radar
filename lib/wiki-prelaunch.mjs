import { WIKI_PRELAUNCH_MODEL_VERSION } from './model-versions.mjs';
import { POLICY_SETS } from './source-registry.mjs';

// Declared in lib/source-registry.mjs. Used to be two identical one-element sets
// here, which is how the same tag ended up defined in nine different files.
const WISHLIST_TAGS = POLICY_SETS.WIKI_PRELAUNCH_WISHLIST;
...
function isWishlistSource(source = {}) {
  return sourceAliases(source).some((value) => WISHLIST_TAGS.has(value));
}
...
export function analyzeWikiPrelaunch(candidate = {}, nowMs = Date.now()) {
  ...
  const hasSteamMomentum = wishlistRankGain >= 3
    || (wishlistAgeDays !== null && wishlistAgeDays <= 7 && wishlistRank > 0 && wishlistRank <= 100);
  const hasSearchFormation = guideSuggestions.length >= 2
    || ['rising', 'breakout'].includes(candidate.trend?.classification);
  const hasDemandAcceleration = hasSteamMomentum && hasSearchFormation;
  const hasGuideDepth = guideSuggestions.length >= 3;
  const hasExternalSpillover = Boolean(social.hasExternalSpillover && social.allowsIndependent);

  let classification = 'weak';
  if (hasWishlistEvidence && releaseWindow.state === 'pre-release' && wishlistRank <= 100 && hasGuideDepth && hasDemandAcceleration && hasExternalSpillover && score >= 60) classification = 'priority';
  ...
  if (hasWishlistEvidence && !hasSteamMomentum) reasons.push('愿望单总排名较高，但近期增长速度尚未得到验证');
  if (hasWishlistEvidence && !hasExternalSpillover) reasons.push('尚未验证多个平台、多个独立创作者的外溢传播');
  if (releaseWindow.state === 'released') reasons.push('游戏已经发售，不再按预发布抢跑机会处理');

  return {
    modelVersion: WIKI_PRELAUNCH_MODEL_VERSION,
...
export { WIKI_PRELAUNCH_MODEL_VERSION };
