...
export function evaluateHealth(report = {}, options = {}) {
  ...
  if (number(report.seoErrors) > 0) {
    findings.push(finding(
      'warning',
      `本轮 SEO 验证失败 ${number(report.seoErrors)} 次`,
      '注意这是**本轮**的失败次数（scan.mjs 每轮重算），不是全池 error 候选数 —— '
        + '看这个数字会以为池子里有那么多坏词。被拦截的已经单独算在 seoBlocked 里，'
        + '所以这里剩下的是真实错误：超时、HTTP 非 200、或响应格式变了。',
    ));
  }
